...
export async function GET() {
  ...
  const urls: string[] = [
    `<url><loc>${SITE_URL}/ca</loc><lastmod>${now}</lastmod><changefreq>daily</changefreq><priority>1.0</priority></url>`,
    `<url><loc>${SITE_URL}/ca/verbos</loc><lastmod>${now}</lastmod><changefreq>weekly</changefreq><priority>0.7</priority></url>`,
    `<url><loc>${SITE_URL}/ca/verbs-catalans</loc><lastmod>${now}</lastmod><changefreq>weekly</changefreq><priority>0.7</priority></url>`,
    ...
    // CA verbs in CA interface
    ...caSlugs.map(({ slug, frequency_rank }) =>
      `<url><loc>${SITE_URL}/ca/verb-catala/${slug}</loc><lastmod>${now}</lastmod><changefreq>monthly</changefreq><priority>${priority(frequency_rank)}</priority></url>`
    ),
    // EN verbs in CA interface
    ...enSlugs.map(({ slug, frequency_rank }) =>
      `<url><loc>${SITE_URL}/ca/verb-angles/${slug}</loc><lastmod>${now}</lastmod><changefreq>monthly</changefreq><priority>${priority(frequency_rank)}</priority></url>`
    ),
  ];
